...
/// ARM instruction classes, in decode-priority order.
const ARM_B: u8 = 0;
const ARM_BX: u8 = 1;
const ARM_SWI: u8 = 2;
const ARM_LDM_STM: u8 = 3;
const ARM_LDR_STR: u8 = 4;
const ARM_SWP: u8 = 5;
const ARM_HALFWORD: u8 = 6;
const ARM_MUL: u8 = 7;
const ARM_MULL: u8 = 8;
const ARM_MRS: u8 = 9;
const ARM_MSR: u8 = 10;
const ARM_DATA_PROC: u8 = 11;
const ARM_UNDEFINED: u8 = 12;

/// Instruction bits the class table is indexed by: 27-20 and 7-4.
#[inline(always)]
const fn arm_key(instr: u32) -> usize {
    (((instr >> 16) & 0xFF0) | ((instr >> 4) & 0xF)) as usize
}

/// The full test for class `k` (exactly the old if-chain's condition).
#[inline(always)]
const fn arm_test(k: u8, i: u32) -> bool {
    match k {
        ARM_B => (i & 0x0E00_0000) == 0x0A00_0000,
        ARM_BX => (i & 0x0FFF_FFF0) == 0x012F_FF10,
        ARM_SWI => (i & 0x0F00_0000) == 0x0F00_0000,
        ARM_LDM_STM => (i & 0x0E00_0000) == 0x0800_0000,
        ARM_LDR_STR => (i & 0x0C00_0000) == 0x0400_0000,
        ARM_SWP => (i & 0x0FB0_0FF0) == 0x0100_0090,
        ARM_HALFWORD => (i & 0x0E00_0090) == 0x0000_0090 && ((i >> 5) & 3) != 0,
        ARM_MUL => (i & 0x0FC0_00F0) == 0x0000_0090,
        ARM_MULL => (i & 0x0F80_00F0) == 0x0080_0090,
        ARM_MRS => (i & 0x0FBF_0FFF) == 0x010F_0000,
        ARM_MSR => (i & 0x0DB0_F000) == 0x0120_F000 && ((i & (1 << 25)) != 0 || (i & 0x0FF0) == 0),
        ARM_DATA_PROC => (i & 0x0C00_0000) == 0,
        _ => false,
    }
}

/// Classes that can match an instruction with the given key bits (bit k =
/// class k), in priority order. Built at compile time: a class is a
/// candidate unless its test is false for every instruction with that key.
static ARM_CANDIDATES: [u16; 4096] = {
    const KEY_BITS: u32 = 0x0FF0_00F0;
    const TESTS: [(u32, u32); 12] = [
        (0x0E00_0000, 0x0A00_0000),
        (0x0FFF_FFF0, 0x012F_FF10),
        (0x0F00_0000, 0x0F00_0000),
        (0x0E00_0000, 0x0800_0000),
        (0x0C00_0000, 0x0400_0000),
        (0x0FB0_0FF0, 0x0100_0090),
        (0x0E00_0090, 0x0000_0090),
        (0x0FC0_00F0, 0x0000_0090),
        (0x0F80_00F0, 0x0080_0090),
        (0x0FBF_0FFF, 0x010F_0000),
        (0x0DB0_F000, 0x0120_F000),
        (0x0C00_0000, 0x0000_0000),
    ];
    let mut t = [0u16; 4096];
    let mut key = 0;
    while key < 4096 {
        // An instruction with these key bits and zeros elsewhere.
        let r = ((key as u32 & 0xFF0) << 16) | ((key as u32 & 0xF) << 4);
        let mut k = 0;
        while k < TESTS.len() {
            let (mask, val) = TESTS[k];
            let mut ok = ((r ^ val) & mask & KEY_BITS) == 0;
            // Extra conditions, evaluated on what the key decides.
            if k == ARM_HALFWORD as usize {
                ok = ok && ((r >> 5) & 3) != 0;
            }
            if k == ARM_MSR as usize {
                // I bit, or bits 7-4 zero (bits 11-8 aren't in the key).
                ok = ok && ((r & (1 << 25)) != 0 || (r & 0xF0) == 0);
            }
            if ok {
                t[key] |= 1 << k;
            }
...
/// Class of an ARM instruction: the first candidate whose full test holds.
#[inline(always)]
fn arm_class(instr: u32) -> u8 {
    let mut c = ARM_CANDIDATES[arm_key(instr)];
    while c != 0 {
...
fn execute_arm(cpu: &mut Arm7Tdmi, mmu: &mut Mmu, instr: u32) -> u32 {
    ...
    // JIT stage 3 (docs/JIT.md): the instruction class comes from a table
    // over bits 27-20 and 7-4 plus the full test of each candidate, in the
    // same order as the old if-chain (see `arm_class`).
    match arm_class(instr) {
...
mod decode_table_tests {
    ...
    fn reference_class(instr: u32) -> u8 {
        (0..ARM_UNDEFINED).find(|&k| arm_test(k, instr)).unwrap_or(ARM_UNDEFINED)
    }

    #[test]
    fn table_matches_the_if_chain() {
        // Every key, with the other bits (19-8, 3-0) set to each mix of
        // 0 / all ones / a fixed pattern per nibble...
        let nibbles = [(16, 0xF), (12, 0xF), (8, 0xF), (0, 0xF)];
        for key in 0..4096u32 {
            let base = ((key & 0xFF0) << 16) | ((key & 0xF) << 4);
            for mix in 0..81u32 {
                let mut instr = base | 0xE000_0000;
                let mut m = mix;
                for (shift, full) in nibbles {
                    instr |= [0, full, 0x5 & full][(m % 3) as usize] << shift;
                    m /= 3;
                }
                assert_eq!(arm_class(instr), reference_class(instr), "{instr:#010x}");
            }
        }
        // ...and a few million random instructions.
    ...
    #[test]
    fn known_encodings() {
        assert_eq!(arm_class(0xEA00_0000), ARM_B);
        assert_eq!(arm_class(0xE12F_FF1E), ARM_BX); // bx lr
        assert_eq!(arm_class(0xEF00_0000), ARM_SWI);
        assert_eq!(arm_class(0xE92D_4000), ARM_LDM_STM); // push {lr}
        assert_eq!(arm_class(0xE591_0000), ARM_LDR_STR);
        assert_eq!(arm_class(0xE101_0092), ARM_SWP);
        assert_eq!(arm_class(0xE1D0_00B0), ARM_HALFWORD); // ldrh
        assert_eq!(arm_class(0xE000_0291), ARM_MUL);
        assert_eq!(arm_class(0xE081_0392), ARM_MULL);
        assert_eq!(arm_class(0xE10F_0000), ARM_MRS);
        assert_eq!(arm_class(0xE169_F000), ARM_MSR); // msr spsr_fc, r0
        assert_eq!(arm_class(0xE1A0_0000), ARM_DATA_PROC); // mov r0, r0
        assert_eq!(arm_class(0xE600_0010), ARM_LDR_STR); // matches the chain's order
    }
}
