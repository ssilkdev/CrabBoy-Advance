...
/// CrabBoy Advance patch: raw game-controller input.
///
/// Upstream winit turns every Android key event into a `KeyboardInput` and
/// marks it handled; gamepad buttons (`KEYCODE_BUTTON_*`) arrive as
/// `Key::Unidentified`, which egui drops, and joystick motion events are
/// treated as touches. Installing a hook routes events whose source is a
/// gamepad, joystick or D-pad to the hook instead.
pub mod gamepad {
    use std::sync::OnceLock;

    /// A raw controller event.
    #[derive(Debug, Clone, Copy)]
    pub enum GamepadEvent {
        /// Android `KEYCODE_*` value and whether it is now held.
        Key { keycode: u32, pressed: bool },
        /// Joystick axes: D-pad hat (`HAT_X/Y`) and left stick (`X/Y`),
        /// each in `-1.0..=1.0`.
        Axes { hat_x: f32, hat_y: f32, x: f32, y: f32 },
    }

    type Hook = Box<dyn Fn(GamepadEvent) + Send + Sync>;
    static HOOK: OnceLock<Hook> = OnceLock::new();

    /// Install the controller hook. Only the first call takes effect.
    pub fn set_gamepad_hook(hook: impl Fn(GamepadEvent) + Send + Sync + 'static) {
        let _ = HOOK.set(Box::new(hook));
    }

    pub(crate) fn dispatch(event: GamepadEvent) -> bool {
        match HOOK.get() {
            Some(hook) => {
                hook(event);
                true
    ...
    // AINPUT_SOURCE_CLASS_JOYSTICK, plus the GAMEPAD / DPAD source bits.
    const SOURCE_CLASS_JOYSTICK: u32 = 0x0000_0010;
    const SOURCE_GAMEPAD_BITS: u32 = 0x0000_0400;
    const SOURCE_DPAD_BITS: u32 = 0x0000_0200;

    pub(crate) fn is_controller_key(source: u32) -> bool {
        source & (SOURCE_GAMEPAD_BITS | SOURCE_DPAD_BITS | SOURCE_CLASS_JOYSTICK) != 0
    }

    pub(crate) fn is_joystick_motion(source: u32) -> bool {
        source & SOURCE_CLASS_JOYSTICK != 0
    }
}
