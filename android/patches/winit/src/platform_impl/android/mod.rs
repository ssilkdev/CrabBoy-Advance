...
use crate::platform::android::gamepad::{self as cb_gamepad, GamepadEvent};
...
impl<T: 'static> EventLoop<T> {
        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        match event {
            // CrabBoy Advance patch: controller sticks/hats go to the gamepad hook.
            InputEvent::MotionEvent(motion_event)
                if cb_gamepad::is_joystick_motion(u32::from(motion_event.source()))
                    && motion_event.pointer_count() > 0 =>
            {
                let p = motion_event.pointer_at_index(0);
                let axes = GamepadEvent::Axes {
                    hat_x: p.axis_value(Axis::HatX),
                    hat_y: p.axis_value(Axis::HatY),
                    x: p.axis_value(Axis::X),
                    y: p.axis_value(Axis::Y),
                };
                if !cb_gamepad::dispatch(axes) {
                    input_status = InputStatus::Unhandled;
                }
            },
            // CrabBoy Advance patch: controller buttons (and the system Back
            // key, which egui has no key for) go to the gamepad hook.
            InputEvent::KeyEvent(key)
                if (cb_gamepad::is_controller_key(u32::from(key.source()))
                    || key.key_code() == Keycode::Back)
                    && !matches!(
                        key.key_code(),
                        Keycode::VolumeUp | Keycode::VolumeDown | Keycode::VolumeMute
                    ) =>
            {
                let pressed = matches!(key.action(), KeyAction::Down);
                let event = GamepadEvent::Key { keycode: u32::from(key.key_code()), pressed };
                if !cb_gamepad::dispatch(event) {
                    input_status = InputStatus::Unhandled;
                }
            },
            InputEvent::MotionEvent(motion_event) => {
