...
impl WinitApp for GlowWinitApp<'_> {
    ...
    fn save_and_destroy(&mut self) {
        if let Some(mut running) = self.running.take() {
            ...
            // CrabBoy patch: on Android the window is already gone when the
            // activity is destroyed; `.window()` panicked here.
            let window_opt = running.glutin.borrow().window_opt(ViewportId::ROOT);
            running.integration.save(running.app.as_mut(), window_opt.as_deref());
